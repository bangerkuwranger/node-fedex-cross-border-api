...
class CartonsDat {
  ...
  getXmlString() {
    ...
    for (let i = 0; i < CartonsDatValues.length; i++) {
      ...
      if (this.hasOwnProperty(CartonsDatValues[i]) && 'function' != typeof CartonsDatValues[i]) {
...
class OrderInformation {
  ...
  getXmlString() {
    ...
    for (let i = 0; i < OrderInformationValues.length; i++) {
...
class ProductsIdDat {
  ...
  getXmlString() {
    ...
    for (let i = 0; i < ProductsIdDatValues.length; i++) {
...
class LandedProduct {
  ...
  getXmlString() {
    ...
    for (let i = 0; i < LandedProductValues.length; i++) {
      ...
      if ('function' != typeof callback || callback.length !== 2) {
        ...
        if ('undefined' == typeof lcRequest || 'object' != typeof lcRequest.items || !(Array.isArray(lcRequest.items)) || 'string' != typeof lcRequest.shipmentDestinationCountry) {
          ...
          if ('object' == typeof lcRequest.optionalArgs) {
            ...
            if ('string' == typeof lcRequest.optionalArgs.orderValuationCurrency && isValidCurrency(lcRequest.optionalArgs.orderValuationCurrency.trim().substring(0,3).toUpperCase())) {

              this.landedCostRequest.orderValuationCurrency = lcRequest.optionalArgs.orderValuationCurrency.trim().substring(0,3).toUpperCase();
            ...
            if ('string' == typeof lcRequest.optionalArgs.requestedCostReturnedCurrency && isValidCurrency(lcRequest.optionalArgs.requestedCostReturnedCurrency.trim().substring(0,3).toUpperCase())) {

              this.landedCostRequest.requestedCostReturnedCurrency = lcRequest.optionalArgs.requestedCostReturnedCurrency.trim().substring(0,3).toUpperCase();
            ...
            if (error) {
              ...
              if (bodyError != 0) {
                ...
                if ('undefined' != typeof justTheBody.dutyCost) {
                  ...
                  bodyObj.dutyCost = parseFloat(xmlNodeValue(justTheBody.dutyCost));
