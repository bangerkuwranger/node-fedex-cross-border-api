...
function xmlNodeValue (nodeObject) {
  if (typeof nodeObject !== 'object' || typeof nodeObject._ === 'undefined') {
    return nodeObject
  } else {
    return nodeObject._
  }
  ...
  client: class FedExCBClientCallback {
    constructor (args) {
      ...
      if (typeof args !== object) {
        throw new TypeError('FECB client constructor requires args to be an object')
      }
          ...
          if (error) {
            ...
            if (bodyError !== 0) {
              ...
              for (let argName in lcResponseArgs) {
                setPropertyIf(justTheBody, bodyObj, argName, lcResponseArgs[argName])
              }
            ...
            if (error) {
              ...
              if (bodyError !== 0) {
                ...
                for (let argName in oResponseArgs) {
                  setPropertyIf(justTheBody, bodyObj, argName, oResponseArgs[argName])
                }
                if (this.stripWrapper) {
            ...
            if (error) {
              ...
              if (bodyError !== 0) {
                ...
                for (let argName in otuResponseArgs) {
                  setPropertyIf(justTheBody, bodyObj, argName, otuResponseArgs[argName])
                }
                if (this.stripWrapper) {
      ...
      if (typeof callback !== 'function' || callback.length !== 2) {
        ...
        if (typeof orOrderNumber !== 'string') {
          ...
          this.orderRemoveRequest = {
            language: this.language,
            partnerKey: this.merchantKey,
            orderNumber: orOrderNumber.trim().substring(0, 100)
          }
