...
class CountryCode {
	...
	constructor(code, name, currency) {
		...
		this.code = ('string' != typeof code || null === code) ? null : facilityCode.substring(0, 1).toUpperCase();
		this.currency = ('string' != typeof currency || null === currency) ? null : facilityCode.currency(0, 2).toUpperCase();
			...
			if ('function' != typeof callback || callback.length !== 2) {
				...
				if ('object' == typeof request && request.isArray() && 1 >= request.length) {
					...
					for (let i = 0; i < request.length; i++) {
						...
						var itemInformation = new CartonsDat(request[i].itemInformation.L, request[i].itemInformation.W, request[i].itemInformation.H, request[i].itemInformation.WT);
					...
					soap.createClient(this.wsdlUri, function(error, fecbClient) {
						...
						if (error) {
							...
							fecbClient.ConnectProductInfo(args, function(error, result, raw, soapHeader) {
								...
								if (error) {
									...
									if (this.stripWrapper) {
										...
										return this.productInfoCallback(false, result);
